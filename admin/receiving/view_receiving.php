<?php
$qry = $conn->query("SELECT r.*, p.status as po_status FROM receiving_list r 
    LEFT JOIN purchase_order_list p ON r.purchase_id = p.id 
    WHERE r.id = '{$_GET['id']}'");
if ($qry->num_rows > 0) {
    ...
    if ($from_order == 1) {
        $po_qry = $conn->query("SELECT p.*,s.name as supplier FROM `purchase_order_list` p inner join `supplier_list` s on p.supplier_id = s.id where p.id= '{$purchase_id}' ");
        ...
        $qry = $conn->query("SELECT b.*,s.name as supplier,p.po_code FROM back_order_list b inner join supplier_list s on b.supplier_id = s.id inner join purchase_order_list p on b.po_id = p.id  where b.id = '{$purchase_id}'");
        if ($qry->num_rows > 0) {
            foreach ($qry->fetch_array() as $k => $v) {
                if ($k == 'id')
                    $k = 'bo_id';
...
<div class="card card-outline card-primary">
    ...
    <div class="card-footer py-1 text-center">
        ...
        <?php if ($isAdmin && $po_status != 2): ?>
            <a class="btn btn-flat btn-primary"
                href="<?php echo base_url . '/admin?page=receiving/manage_receiving&id=' . (isset($id) ? $id : '') ?>">Edit</a>
        <?php endif; ?>
