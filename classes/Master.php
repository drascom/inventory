...
class Master extends DBConnection
{
	...
	function save_client()
	{
		...
		if ($check > 0) {
			$resp['status'] = 'failed';
			$resp['msg'] = "client Name already exist.";
			return json_encode($resp);
			exit;
		}
		if (empty($id)) {
		...
		if ($save) {
			$resp['status'] = 'success';
			if (empty($id)) {
				$res['msg'] = "New client successfully saved.";
				$id = $this->conn->insert_id;
			} else {
				$res['msg'] = "client successfully updated.";
			}
			$this->settings->set_flashdata('success', $res['msg']);
		} else {
			$resp['status'] = 'failed';
	...
	function get_supplier_pos()
	{
		extract($_POST);

		$qry = $this->conn->query("SELECT * FROM purchase_order_list WHERE supplier_id = '{$supplier_id}' AND (status = 1 OR status = 2) ORDER BY date_created DESC");
		$html = '<option value=""></option>';

		while ($row = $qry->fetch_assoc()) {
			$html .= '<option value="' . $row['id'] . '">PO-' . $row['po_code'] . ' (' . date("Y-m-d", strtotime($row['date_created'])) . ')</option>';
		}
	...
	function get_po_items()
	{
		...
		$qry = $this->conn->query("SELECT pi.*, i.name, i.description FROM po_items pi 
		...
		while ($row = $qry->fetch_assoc()) {
			...
			<tr>
				<td class="text-center">
					<button class="btn btn-sm btn-danger" onclick="removeRow(this)" type="button"><i class="fa fa-times"></i></button>
				</td>
				<td class="text-center">
					<input type="number" class="form-control" name="qty[]" max="' . $row['quantity'] . '" value="0">
	...
	function save_return()
	{
		...
		$data = "";
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('id', 'item_id', 'qty', 'unit', 'price')) && !is_array($_POST[$k])) {
				if (!empty($data))
					$data .= ", ";
				$data .= " `{$k}` = '{$v}' ";
			}
		...
		if ($save) {
			...
			$data = "";
			foreach ($item_id as $k => $v) {
				$total = floatval($qty[$k]) * floatval($price[$k]);
				if (!empty($data))
					$data .= ", ";
				$data .= "('{$return_id}','{$v}','{$qty[$k]}','{$price[$k]}','{$unit[$k]}','{$total}')";
			}

			if (!empty($data)) {
				$this->conn->query("DELETE FROM `return_items` where return_id = '{$return_id}'");
				$save = $this->conn->query("INSERT INTO `return_items` (`return_id`,`item_id`,`quantity`,`price`,`unit`,`total`) VALUES {$data}");
			}
		} else {
			$resp['status'] = 'failed';
